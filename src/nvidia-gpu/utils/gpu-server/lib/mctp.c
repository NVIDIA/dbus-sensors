...
nsm_requester_rc_t nsm_recv_any(mctp_eid_t eid, int mctp_fd,
                                ...
                                uint8_t* tag)
{
    nsm_requester_rc_t rc = mctp_recv(eid, mctp_fd, nsm_resp_msg, resp_msg_len,
                                      tag);
    ...
    if (*resp_msg_len < (sizeof(struct nsm_msg_hdr) + NSM_RESPONSE_MIN_LEN))
    {
        free(*nsm_resp_msg);
        *nsm_resp_msg = NULL;
...
{
    uint8_t tag;
    nsm_requester_rc_t rc = nsm_recv_any(eid, mctp_fd, nsm_resp_msg,
                                         resp_msg_len, &tag);
    if (rc != NSM_REQUESTER_SUCCESS)
    {
        return rc;
