        ...
        sensorsChanged)
{
    auto getter = std::make_shared<GetSensorConfiguration>(
                 ...
                 sensor : sensorConfigurations)
        {
            ...
            int totalGPU = std::visit(VariantToIntVisitor(),
                                      findTotalGPU->second);
            ...
            auto findGPUInterface = baseConfiguration->second.find("Interface");
            if (findGPUInterface == baseConfiguration->second.end())
            {
                std::cerr << "could not determine configuration D-bus interface"
                          << "\n";
                continue;
            }
            ...
            auto findGPUProperty = baseConfiguration->second.find("Property");
            if (findGPUProperty == baseConfiguration->second.end())
            {
                std::cerr << "could not determine configuration D-bus property"
                          << "\n";
                continue;
            }
            ...
            sensorConstruct = std::make_shared<GPUStatus>(
                objectServer, dbusConnection, sensorName, gpuService, gpuObject,
                gpuInterface, gpuProperty, totalGPU, *interfacePath);
        }
    });
