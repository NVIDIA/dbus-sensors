...
// Drive disabled_main_heartbeat all the way to gIo.run() so the periodic
// "alarm" steady_timer lambda executes. With request_name() mocked to succeed
// and gRunning=false, the first timer expiry takes the termination branch
// (logs, calls gIo.stop(), returns) which unwinds gIo.run() deterministically,
// so main returns 0. This covers the otherwise-unreached alarm lambda inside
// disabled_main_heartbeat.
TEST(DisabledMainHeartbeatRun, alarmTimerStopsIoContextWhenNotRunning)
{
    int fds[2]{-1, -1};
    ASSERT_EQ(pipe(fds), 0);
    gFakeSdBusFd = fds[0];
    gMockSdBusDefault = true;
    gMockSdBusRequestName = true; // request_name() succeeds -> reach gIo.run()
    gMockSdBusRequestNameResult = 0;
    gRunning = false;             // alarm lambda takes the stop-io branch
    ...
    // Restore shared global state for subsequent tests.
    gIo.restart();
    gMockSdBusDefault = false;
    gMockSdBusRequestName = false;
    gRunning = true;
