...
// PCI vendor id for Nvidia OCP accelerator management messages. Defined locally
// to keep this requester independent of the nvidia-gpu headers.
constexpr uint16_t nvidiaPciVendorId = 0x10de;

static std::optional<uint8_t> getIid(std::span<const uint8_t> buffer)
{
    if (peci_mctp::isIntelPeciMessage(buffer))
    {
        return peci_mctp::getIid(buffer);
    }
    if (ocp::accelerator_management::isOcpAcceleratorMessage(buffer,
                                                             nvidiaPciVendorId))
    {
...
static std::optional<bool> getRequestBit(std::span<const uint8_t> buffer)
{
    if (peci_mctp::isIntelPeciMessage(buffer))
    {
        return peci_mctp::isRequestMessage(buffer);
    }
    if (ocp::accelerator_management::isOcpAcceleratorMessage(buffer,
                                                             nvidiaPciVendorId))
    {
...
static std::optional<bool> getDatagramBit(std::span<const uint8_t> buffer)
{
    if (peci_mctp::isIntelPeciMessage(buffer))
    {
        return peci_mctp::getDatagramBit(buffer);
    }
    if (ocp::accelerator_management::isOcpAcceleratorMessage(buffer,
                                                             nvidiaPciVendorId))
    {
...
MctpRequester::MctpRequester(boost::asio::io_context& ctx, uint8_t messageType,
                                   ...
                                   const size_t length)
{
    ...
    if (!optionalIid || !isRq || !isDatagram)
    {
        ...
        // drop this packet on the floor and rebind receive
    ...
    uint8_t eid, const boost::system::error_code& ec, size_t /* length */)
{
    ...
    expiryTimer.async_wait([weak = weak_from_this(),
        ...
        if (ec != boost::asio::error::operation_aborted)
        {
            ...
            self->handleResult(eid, std::make_error_code(std::errc::timed_out),
                               {});
        }
    });
}
...
{
    // Cancellation is expected when the socket is torn down (e.g. host power
    // cycle re-instantiates the requester); it is not fatal.
    return ec &&
                                 ...
                                 std::span<const uint8_t> buffer)
{
    ...
    if (queue.empty())
    {
        // A receive/send-path error can land here for an eid with no request
        // outstanding (e.g. a cancelled socket op). Nothing to complete.
        startReceive();
        return;
    }
                                                      ...
                                                      uint8_t iid)
{
    if (peci_mctp::isIntelPeciMessage(buffer))
    {
        return peci_mctp::injectIid(buffer, iid);
    }
    if (ocp::accelerator_management::isOcpAcceleratorMessage(buffer,
                                                             nvidiaPciVendorId))
    {
...
void MctpRequester::processQueue(uint8_t eid)
{
    ...
    MctpAsioEndpoint sendEndPoint(eid, msgType);
    boost::asio::const_buffer buf(req.data(), req.size());
    mctpSocket.async_send_to(
        buf, sendEndPoint.endpoint,
        [weak = weak_from_this(),
         eid](const boost::system::error_code& ec, size_t length) {
            if (auto self = weak.lock())
            {
                self->handleSendMsgCompletion(eid, ec, length);
            }
        });
}
