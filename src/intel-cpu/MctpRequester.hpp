...
namespace mctp
{
class MctpRequester : public std::enable_shared_from_this<MctpRequester>
{
  public:
    ...
    using EventCallback =
        std::function<void(uint8_t eid, std::span<const uint8_t>)>;

    MctpRequester(boost::asio::io_context& ctx, uint8_t messageType,
                  EventCallback eventCallback = {});

    // Begin receiving. Must be called once, after the object is owned by a
    // shared_ptr (the async handlers hold a weak_from_this()).
    void start();
