        ...
        sensorsChanged)
{
    auto getter = std::make_shared<GetSensorConfiguration>(
                 ...
                 sensor : sensorConfigurations)
        {
            ...
            int totalPSU = std::visit(VariantToIntVisitor(),
                                      findTotalPSU->second);
            ...
            {
                std::cerr << "could not determine configuration redundantPSU"
                          << "\n";
                continue;
            }
            int redundantPSU = std::visit(VariantToIntVisitor(),
                                          findRedundantPSU->second);

            auto findSufficientPSU =
                baseConfiguration->second.find("SufficientPSU");
            if (findSufficientPSU == baseConfiguration->second.end())
            {
                std::cerr << "could not determine configuration sufficientPSU"
                          << "\n";
                continue;
            }
